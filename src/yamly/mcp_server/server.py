...
from yamly.mcp_server.tools import call_tool, get_tool_definitions
...
def _create_server(config: MCPServerConfig | None = None) -> Server:
    ...
    server = Server("yamly-mcp-server", lifespan=lifespan_wrapper)
...
async def run_server(config: MCPServerConfig | None = None) -> None:
    ...
    logger.info(f"Starting yamly MCP server v{__version__}")
